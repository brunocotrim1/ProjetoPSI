...
export class SchedulereunionComponent implements OnInit {
  ...
  onChangeDurationReunion(){
    if(this.f["reunionTime"].errors){
      //"^([0-1]?[0-7]):(30|00)|([0-1]?8):00$"
      //"^00:00$"
      this.isZero = false;
      this.isBiggerThanEight = false;
      this.isNotMultiple30 = false;
      this.isZero = /^0?0:00$/.test(this.f["reunionTime"].value)
      this.isBiggerThanEight = /^(0?8):30|(0?9):[0-5][0-9]|([1-9][0-9]):[0-5][0-9]$/.test(this.f["reunionTime"].value)
      this.isNotMultiple30 = !(/^[0-9]?[0-9]:(30|00)$/.test(this.f["reunionTime"].value))
    } else {
      this.isZero = false;
      this.isBiggerThanEight = false;
      this.isNotMultiple30 = false;
